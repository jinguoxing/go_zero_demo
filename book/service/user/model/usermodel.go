...
func (m *defaultUserModel) Insert(data User) (sql.Result, error) {
	...
	ret, err := m.ExecNoCache(query, data.Gender, data.Number, data.Name, data.Password)
...
func (m *defaultUserModel) FindOneByNumber(number string) (*User, error) {
	userIdKey := fmt.Sprintf("%s%v", cacheUserIdPrefix, number)
	var resp User
	err := m.QueryRow(&resp, userIdKey, func(conn sqlx.SqlConn, v interface{}) error {
		query := fmt.Sprintf("select %s from %s where `number` = ? limit 1", userRows, m.table)
		return conn.QueryRow(v, query, number)
	})
	switch err {
	case nil:
		return &resp, nil
...
func (m *defaultUserModel) Update(data User) error {
	...
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Gender, data.Number, data.Name, data.Password, data.Id)
	}, userIdKey)
	return err
}
